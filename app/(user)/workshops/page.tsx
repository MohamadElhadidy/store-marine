...
function Suppliers() {
    ...
    const { isLoading, error, data, isFetching } = useQuery(['workshops'], GetData,{
        onSuccess: (data) => {
          console.log(data)
    ...
    const columns = [
      ...
      columnHelper.accessor('code', {
        header: 'كود الورشة الخارجية',
        cell: info => info.getValue(),
        footer: info => info.column.id,
      }),
      columnHelper.accessor('name', {
        header: 'اسم الورشة الخارجية',
        cell: info => info.getValue(),
        footer: info => info.column.id,
      }),
    ...
    return (
      <>
            ...
            <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-xl rounded  ">
              <div className="rounded-t mb-0 px-4 py-3 border-0 flex flex-wrap items-center justify-between w-full ">
                <div className="relative  px-4">
                  <h3 className="font-semibold text-xl text-blueGray-700 underline underline-offset-[6px] decoration-blue-800 decoration-[4px]">تقرير الورش الخارجية</h3>
                </div>
                <div className="relative  px-4">
                  ...
                  >
                    ...
                    <span className='text-sm'>إضافة ورشة الخارجية</span>
                  </button>
                ...
                <table className="items-center bg-transparent w-full border-collapse ">
                  ...
                  <tbody>
                    {table.getHeaderGroups().map(headerGroup => (
                      <tr key={headerGroup.id}>
                        {headerGroup.headers.map(header => (
                          <td key={header.id} className="border-t-0 py-1 align-middle border-l-0 border-r-0 text-sm text-center font-bold">
                            {header.column.getCanFilter() ? (
                              <div>
                                <Filter column={header.column} table={table} />
                              </div>
        ...
        <AnimatePresence
          ...
          mode='wait'
        >
          {modalDelete && <Delete handleClose={() => setModalDelete(false)} data={id} text="هل أنت متأكد من حذف الورشة الخارجية ؟ " afterText='تم حذف الورشة الخارجية بنجاح' />}
