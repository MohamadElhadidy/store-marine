...
function Delete({ handleClose, text, data,afterText }) {
  const {id, name} = data
          ...
          <div className=" bg-white rounded-lg ">
            ...
            <div className="p-6 text-center">
              <svg aria-hidden="true" className="mx-auto mb-4 w-14 h-14 text-black " fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                {mutation.isLoading && <h3 className="mb-5 text-xl font-bold text-black  "><span className="text-red-600">جاري حذف الورشة الخارجية....</span></h3>}
              <h3 className="mb-5 text-lg font-bold text-black  "><span className="text-red-600">{name}</span><br/>{text}</h3>
                ...
                {mutation.isLoading ? <svg aria-hidden="true" className=" absolute left-0 w-10 h-8 text-white animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <button onClick={handleClose} data-modal-toggle="popup-modal" type="button" className="focus:ring-4 focus:outline-none  rounded-lg border  text-sm font-medium px-5 py-2.5  focus:z-10 bg-gray-700 text-white border-gray-500 hover:text-white hover:bg-gray-600 focus:ring-gray-600">إلغاء</button>
            </div> : 
